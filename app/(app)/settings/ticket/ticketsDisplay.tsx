    ...
    return (
        <div className="flex flex-col gap-2">
            {tickets?.length === 0 ?
                ...
                tickets.map((ticket) => (
                    <Link href={`/settings/ticket/${ticket.id}`} key={ticket.id} className="flex justify-between px-4 py-2 duration-100 rounded-md border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground gap-4 cursor-pointer">
